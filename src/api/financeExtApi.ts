...
export const listAccounts = () => apiClient.get<GLAccount[]>('/api/v1/gl/accounts').then(r => Array.isArray(r.data) ? r.data : [])
export const createAccount = (b: Omit<GLAccount, 'id'>) => apiClient.post<GLAccount>('/api/v1/gl/accounts', b).then(r => r.data)
export const listJournalEntries = (status?: string) => apiClient.get<JournalEntry[]>('/api/v1/gl/journal-entries', { params: status ? { status } : {} }).then(r => Array.isArray(r.data) ? r.data : [])
...
export const listAssets = () => apiClient.get<FixedAsset[]>('/api/v1/fixed-assets/').then(r => Array.isArray(r.data) ? r.data : [])
...
export const listBudgets = (fy?: string) => apiClient.get<BudgetLine[]>('/api/v1/budgets/', { params: fy ? { fy } : {} }).then(r => Array.isArray(r.data) ? r.data : [])
