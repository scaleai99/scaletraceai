...
export async function getCustomer360(customerId: string): Promise<Customer360> {
  const { data } = await apiClient.get<Customer360>(`${V1}/customers/${customerId}/dashboard-360`)
  return data
}

export async function markQuotationLost(quotationId: string, body: MarkLostRequest) {
  const { data } = await apiClient.patch(`${V1}/quotations/${quotationId}/mark-lost`, body)
  return data
}

export async function getCompetitiveAnalysis(): Promise<CompetitiveAnalysis> {
  const { data } = await apiClient.get<CompetitiveAnalysis>(`${V1}/quotations/dashboard/competitive-analysis`)
  return data ?? { win_loss_by_reason: {}, avg_price_gap_pct_on_price_loss: null, top_competitors: [] }
}
