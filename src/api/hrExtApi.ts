...
export const listRuns = () => apiClient.get<PayrollRun[]>('/api/v1/payroll/runs').then(r => r.data)
...
export const listAppraisals = (status?: string) => apiClient.get<Appraisal[]>('/api/v1/appraisals/', { params: status ? { status } : {} }).then(r => r.data)
...
export const listReqs = () => apiClient.get<JobRequisition[]>('/api/v1/recruitment/requisitions').then(r => r.data)
...
export const listCandidates = (requisition_id?: string) => apiClient.get<Candidate[]>('/api/v1/recruitment/candidates', { params: requisition_id ? { requisition_id } : {} }).then(r => r.data)
...
export const listLeave = (status?: string) => apiClient.get<LeaveRequest[]>('/api/v1/ess/leave-requests', { params: status ? { status } : {} }).then(r => r.data)
